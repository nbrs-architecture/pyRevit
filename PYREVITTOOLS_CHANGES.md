# PyRevitTools Modifications

Living document tracking every change to `extensions/pyRevitTools.extension/`.
Update this file each time you modify our tools. It is used to reconcile changes
if an upstream core sync ever overlaps with our files.

## Added Tools

| Date | Tool path | Description |
|------|-----------|-------------|
| 2026-09-07 | `extensions/pyRevitTools.extension/pyRevit.tab/Selection.panel/Pick.splitpushbutton/Pick Linked Model Elements.pushbutton` | New pick tool: click-selects elements inside linked models (not the link instances). Uses `ObjectType.LinkedElement` pick + `Selection.SetReferences` (Revit 2023+); shows a notice on older Revit |
| 2026-09-04 | `extensions/pyRevitTools.extension/pyRevit.tab/Project.panel/Wipe.pulldown/Wipe Unused Project Parameters.pushbutton` | Finds project parameters (shared + non-shared) with no value on any element (native `CreateHasValueParameterRule` filter, instance + type, Revit 2022+) and lets you multi-select which to wipe |
| 2026-09-04 | `extensions/pyRevitTools.extension/pyRevit.tab/Project.panel/Wipe.pulldown/Wipe Unused Scope Boxes.pushbutton` | Collects scope boxes not used by any view (crop), level, or grid and lets you multi-select which to wipe |
| 2026-08-20 | `extensions/pyRevitTools.extension/pyRevit.tab/Modify.panel/edit2.stack/ReName.pushbutton` | New multipurpose find & replace renamer for family names, views, sheets, filters, and view templates |

## Modified Tools

| Date | Tool path | Change | Reason |
|------|----------|--------|--------|
| 2026-09-21 | `extensions/pyRevitTools.extension/pyRevit.tab/Analysis.panel/Tools.stack/Inspect.pulldown/ListFamilySizeCreator.pushbutton/ListFamilySizeCreator_script.py` | The temporary family save now builds a safe, unique `.rfa` path via a new `make_temp_file_path()` helper (recreates the temp dir if it vanished, replaces Windows-illegal characters, strips trailing dots/spaces, prefixes reserved device names, caps the name length, and falls back to `<name>_n.rfa` when a leftover file is locked). `SaveAs`, `Close(False)` and `os.remove` are wrapped in try/except so one unsaveable family no longer aborts the whole run — the family is logged, listed as `N/A`, and a summary line reports how many could not be measured. The saved path is read back from `fam_doc.PathName` | `os.path.join(temp_dir, fam_doc.Title)` used a Revit name as a file name with no `.rfa` extension: a `:` made the tail an NTFS alternate-data-stream reference (`FileNotFoundException` / `COleException 0x80030002` = `STG_E_FILENOTFOUND`), an empty `Title` (or a leftover locked temp file) gave `InvalidOperationException: Saving failed.`, and the missing extension meant Revit saved to `<name>.rfa` while the script measured and deleted the extension-less path |
| 2026-09-10 | `extensions/pyRevitTools.extension/pyRevit.tab/Drawing Set.panel/Sheets.pulldown/List TitleBlocks on Sheets.pushbutton/script.py` | Reworked the output into a `print_table` report with SHEET / TITLEBLOCK / VIEWS / SCALE columns (sheet + element names hyperlinked, sheets sorted by sheet number, sheets without titleblocks skipped, alert when nothing found) | Flat one-line-per-sheet print was hard to scan on large sets, and the sheet scale + placed views are needed when batch-editing titleblocks |
| 2026-09-07 | `extensions/pyRevitTools.extension/pyRevit.tab/Selection.panel/Pick.splitpushbutton/Pick Linked Model Elements.pushbutton/script.py` | Selects each linked element as a WHOLE: picked references are rebuilt via `DB.Reference(linked_element).CreateLinkReference(link_instance)` before `Selection.SetReferences` | Raw pick references can point at the clicked face/edge, so Revit's Filter dialog listed some elements under "*Partially selected elements" |
| 2026-08-20 | `pyrevitlib/pyrevit/revit/db/query.py` (`get_sheet_tblocks`) | Performance: replaced view-scoped collector with a single global collector filtered by `OwnerViewId` | View-scoped collector forced Revit to regenerate every sheet when querying titleblocks |
| 2026-08-20 | `extensions/pyRevitTools.extension/pyRevit.tab/Modify.panel/edit2.stack/ReName.pushbutton/script.py` | Fixed for Revit 2026: replaced `ElementId.IntegerValue` with `pyrevit.compat.get_elementid_value_func()` (version-safe, works 2022+) | `ElementId.IntegerValue` removed in Revit 2026 |
| 2026-08-20 | `extensions/pyRevitTools.extension/pyRevit.tab/Modify.panel/edit2.stack/ReValue.pushbutton` | Disabled ReValue: renamed `ReValue.pushbutton` to `ReValue.disable` | Tool not needed on this deployment; re-enable by renaming back |
| 2026-08-17 | `extensions/pyRevitTools.extension/pyRevit.tab/Drawing Set.panel/Print Sheets.pushbutton` | Renamed bundle folder to `.Print Sheets.disabled` (non-component postfix + dot-prefix) so both the Python parser and the new C# loader skip it — button hidden from the Drawing Set panel, all assets preserved | Tool not needed on this deployment; re-enable by renaming back |
| 2026-08-17 | `extensions/pyRevitTools.extension/pyRevit.tab/Project.panel/Wipe.pulldown/` | Disabled in-use components wipe: renamed the seven Wipe `.pushbutton` folders to `.disable` | Tools not needed on this deployment; re-enable by renaming back |
| 2026-08-17 | `extensions/pyRevitTools.extension/pyRevit.tab/Selection.panel/mema.stack` and `memo.stack` | Disabled memo and mema stacks: renamed to `mema.disable` / `memo.disable` | Stacks not needed on this deployment; re-enable by renaming back |

## Removed Tools

| Date | Tool path | Reason |
|------|----------|--------|
| _None yet_ | | |
